...
nrow(RashnaTitanic)
ncol(RashnaTitanic)
colnames(RashnaTitanic)
rownames(RashnaTitanic)
class(RashnaTitanic)
...
Sex <- RashnaTitanic[["Sex"]]
Age <- RashnaTitanic[["Age"]]
...
new_dataframe <- data.frame(Sex = Sex, Age = Age)
...
subset_data <- subset(RashnaTitanic, Freq > 25)
...
ggplot(RashnaTitanic, aes(Survived, Freq)) +
   geom_col()
